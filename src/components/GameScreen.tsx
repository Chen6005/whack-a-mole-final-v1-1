...
import { getLevel, getLevelPassLabel } from "../game/levels";
...
export function GameScreen({ state, onHit, onMiss, onPause, onResume, onRestart, onHome }: GameScreenProps) {
  ...
  return (
    <main className={`screen screen--game ${state.shakeId > 0 ? "can-shake" : ""}`} key={state.shakeId}>
      ...
      {levelConfig && <div className="level-objective"><strong>LV.{levelConfig.id} {levelConfig.name}</strong><span>{getLevelPassLabel(levelConfig)}</span></div>}
